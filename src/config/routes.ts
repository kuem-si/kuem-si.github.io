...
export const ROUTES: RouteEntry[] = [
  ...
  },
  {
    ...
    nav: false,
    labels: {
      sl: "Arhitektura in svetovanje",
      en: "Architecture and consulting",
    },
    title: {
  ...
  },
  {
    ...
    nav: false,
    labels: {
      sl: "DevOps in platformni inzeniring",
...
const routeMap = new Map<RouteId, RouteEntry>(
  ROUTES.map((route) => [route.id, route]),
);

export const MAIN_NAV: RouteId[] = [
  "home",
  "services",
  "solutions",
  "about",
...
export function getNavItems(
  locale: Locale,
): Array<{ id: RouteId; href: string; label: string }> {
...
export function getAlternatePaths(routeId: RouteId): Record<Locale, string> {
  return Object.fromEntries(
    SUPPORTED_LOCALES.map((locale) => [
      locale,
      buildLocalizedPath(locale, routeId),
    ]),
  ) as Record<Locale, string>;
}

export function buildStaticSlugParam(
  locale: Locale,
  routeId: RouteId,
): string | undefined {
  const segments = buildLocalizedSegments(locale, routeId);
